...
//ran on startup uses color specified in config
void enemy_scanner::initColor()
{
    ...
    else
    {
        ...
        BLUE_DOMINANCE_THRES[0] = 1.0f;
        BLUE_DOMINANCE_THRES[1] = 1.0f;
    }
}
//checks if the current pixel passed in is the right color
...
int enemy_scanner::find_enemy_outline_bottom(const Image& image) const
{
    ...
    while ((z < height && z > 0) && thread1_flag == 0)
    {
